...
moz.defaults.bar={y_accessor:'factor',x_accessor:'value',baseline_accessor:null,predictor_accessor:null,predictor_proportion:5,dodge_acessor:null,binned:true,padding_percentage:0,outer_padding_percentage:.1,height:500,top:20,bar_height:20,left:70}
moz.defaults.missing={top:0,bottom:0,right:0,left:0,width:350,height:220,missing_text:'Data currently missing or unavailable'}
...
if((version[0]<2&&version[1]<9)||(version[0]==1&&version[1]==9&&version[2]<1)){throw new Error('Bootstrap\'s JavaScript requires jQuery version 1.9.1 or higher')}}(jQuery);+function($){'use strict';var Tooltip=function(element,options){this.type=this.options=this.enabled=this.timeout=this.hoverState=this.$element=null
...
function y_axis(args){var svg=d3.select($(args.target).find('svg').get(0));var g;var min_y,max_y;args.scalefns.yf=function(di){return args.scales.Y(di[args.y_accessor]);}
...
if(min_y>=0&&!args.min_y&&!args.use_data_y_min){min_y=0;}
...
max_y=max_y*args.inflator;if(!args.min_y&&args.use_data_y_min){min_y=min_y/args.inflator;}
...
$(args.target).find('svg .y-axis').remove();if(!args.y_axis)return this;g=svg.append('g').classed('y-axis',true).classed('y-axis-small',args.use_small_class);if(args.y_label){g.append('text').attr('class','label').attr('x',function(){return-1*(args.top+args.buffer+
...
var svg=d3.select($(args.target).find('svg').get(0));var g=svg.append('g').classed('y-axis',true).classed('y-axis-small',args.use_small_class);if(!args.y_axis)return this;g.selectAll('text').data(args.categorical_variables).enter().append('svg:text').attr('x',args.left).attr('y',function(d){return args.scales.Y(d)+args.scales.Y.rangeBand()/2
...
function x_axis(args){'use strict';var svg=d3.select($(args.target).find('svg').get(0));var g;var min_x;var max_x;args.scalefns.xf=function(di){return args.scales.X(di[args.x_accessor]);}
...
var svg=$(args.target).find('svg');if((svg.find('.main-line').length>0&&args.chart_type!='line')||(svg.find('.points').length>0&&args.chart_type!='point')||(svg.find('.histogram').length>0&&args.chart_type!='histogram')){$(args.target).empty();}
...
return this;}
var button_layout=function(target){'use strict';this.target=target;this.feature_set={};this.public_name={};this.sorters={};this.manual=[];this.manual_map={};this.manual_callback={};this.data=function(data){this._data=data;return this;}
...
if(args.area&&!args.use_data_y_min&&!args.y_axis_negative&&args.data.length<=1){if($(args.target).find('svg path.area'+(line_id)+'-color').length>0){d3.selectAll($(args.target).find('svg path.area'+(line_id)+'-color')).transition().duration(function(){return(args.transition_on_update)?1000:0;}).attr('d',area(args.data[i]));}
...
+args.scales.Y(d[args.y_accessor]);}).rollup(function(v){return v[0];}).entries(d3.merge(args.data.map(function(d){return d;}))).map(function(d){return d.values;});g.selectAll('path').data(voronoi(data_nested)).enter().append('path').attr("d",function(d){return"M"+d.join("L")+"Z";}).datum(function(d){return d.point;}).attr('class',function(d){if(args.linked){var v=d[args.x_accessor];var formatter=d3.time.format('%Y-%m-%d');return'line'+d['line_id']+'-color '+'roll_'+formatter(v);}
...
this.mainPlot=function(){var svg=d3.select($(args.target).find('svg').get(0));var g;$(svg).find('.histogram').remove();var g=svg.append("g").attr("class","histogram");var bar=g.selectAll(".bar").data(args.data[0]).enter().append("g").attr("class","bar").attr("transform",function(d){return"translate("+args.scales.X(d[args.x_accessor]).toFixed(2)
...
return this}
this.mainPlot=function(){var svg=d3.select($(args.target).find('svg').get(0));var g;g=svg.append('g').classed('points',true);var pts=g.selectAll('circle').data(args.data[0]).enter().append('svg:circle').attr('class',function(d,i){return'path-'+i;}).attr('cx',args.scalefns.xf).attr('cy',args.scalefns.yf);if(args.color_accessor!=null){pts.attr('fill',args.scalefns.color);pts.attr('stroke',args.scalefns.color);}
else{pts.classed('points-mono',true);}
if(args.size_accessor!=null){pts.attr('r',args.scalefns.size);}
else{pts.attr('r',args.point_size);}
return this;}
this.rollover=function(){var svg=d3.select($(args.target).find('svg').get(0));if($(args.target+' svg .active_datapoint').length>0){$(args.target+' svg .active_datapoint').remove();}
...
this.display=function(){var this_column;var args=this.args;chart_title(args);var target=args.target;var table=d3.select(target).append('table').classed('data-table',true);var colgroup=table.append('colgroup');var thead=table.append('thead').classed('data-table-thead',true);var tbody=table.append('tbody');var this_column;var tr,th,td_accessor,td_type,th_text,td_text,td;var col;tr=thead.append('tr').classed('header-row',true);for(var h=0;h<args.columns.length;h++){var this_col=args.columns[h];td_type=this_col.type;th_text=this_col.label;th_text=th_text==undefined?'':th_text;tr.append('th').classed('data-table-th',true).style('width',this_col.width).style('text-align',td_type=='title'?'left':'right').text(th_text);}
for(var h=0;h<args.columns.length;h++){col=colgroup.append('col');if(args.columns[h].type=='number'){col.attr('align','char').attr('char','.');}}
for(var i=0;i<args.data.length;i++){tr=tbody.append('tr');for(var j=0;j<args.columns.length;j++){this_column=args.columns[j];td_accessor=this_column.accessor;td_text=args.data[i][td_accessor];td_type=this_column.type;if(td_type=='number'){if(this_column.hasOwnProperty('round')&&!this_column.hasOwnProperty('format')){td_text=d3.format('0,.'+this_column.round+'f')(td_text);}
...
td=tr.append('td').classed('data-table',true).classed('table-number',td_type=='number').classed('table-title',td_type=='type').classed('table-text',td_type=='text').style('width',args.columns[j].width).style('font-size',args.columns[j].font_size).style('text-align',td_type=='title'||td_type=='text'?'left':'right');if(td_type=='title'){td.append('div').text(td_text);if(args.columns[j].hasOwnProperty('secondary_accessor')){td.append('div').text(args.data[i][args.columns[j].secondary_accessor]).classed("secondary-title",true)}}else{td.text(td_text);}}}
return this;}
return this;}
...
svg.classed('missing',true);svg.append('rect').attr('class','missing-pane').attr('x',args.left).attr('y',args.top).attr('width',args.width-(args.left*2)).attr('height',args.height-(args.top*2));svg.selectAll('.missing_text').data([args.missing_text]).enter().append('text').attr('class','missing-text').attr('x',args.width/2).attr('y',args.height/2).attr('dy','.50em').attr('text-anchor','middle').text(args.missing_text)
return this;}
this.init(args);return this;}
